...
const initialState: BlogState = {
  favoriteBlogs: new Map<string, number[]>(),
  allLikedBlogs: new Map<string, number[]>(),
  allBlogs: [],
  myBlogs: [],
  filteredBlogs: [],
  BlogById: null,
...
const blogSlice = createSlice({
  ...
  reducers: {
    ...
    addFavorite: (
      state,
      action: PayloadAction<{ username: string; blogId: number }>
    ) => {
    ...
    updateBlog: (state, action: PayloadAction<Omit<Blog, "likeCount">>) => {
      const blogIndex = state.allBlogs.findIndex(
        (b) => b.id === action.payload.id
      );
      if (blogIndex !== -1) {
        state.allBlogs[blogIndex] = {
          ...state.allBlogs[blogIndex],
          ...action.payload,
        };
        localStorage.setItem("allBlogs", JSON.stringify(state.allBlogs));
    ...
    filteredBlogById: (state, action: PayloadAction<{ blogId: number }>) => {
      state.BlogById =
        state.allBlogs.find((b) => b.id === action.payload.blogId) || null;
    ...
    removeLikedBlog: (
      ...
      if (state.allLikedBlogs.has(action.payload.username)) {
        const userLikedBlogs =
          state.allLikedBlogs.get(action.payload.username) || [];
        state.allLikedBlogs.set(
          action.payload.username,
          userLikedBlogs.filter((id) => id !== action.payload.blogId)
        );
